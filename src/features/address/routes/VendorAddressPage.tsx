...
import { VendorAddressForm } from '../components/VendorAddressForm';

const columnHelper = createColumnHelper<Address>();

export const VendorAddressPage: React.FC = () => {
  ...
  useEffect(() => {
    // Try to get vendor ID from URL or context
    const urlParams = new URLSearchParams(window.location.search);
    const idParam = urlParams.get('vendorId');
    if (idParam) {
      setVendorId(parseInt(idParam));
      fetchVendorAddresses(parseInt(idParam));
    }
  }, []);

  const fetchVendorAddresses = async (vendorId: number) => {
    try {
      setLoading(true);
      const res = await addressApi.getByEntity('VENDOR', vendorId);
  ...
  return (
    <div className="p-2">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h1 className="text-2xl font-bold">Vendor Addresses</h1>
          <p className="text-sm text-muted-foreground">
            Manage addresses for vendors
          </p>
        </div>
      ...
      {vendorId === null ? (
        <div className="alert alert-info">
          Please select a vendor to view their addresses.
        </div>
      ) : (
        <>
          <DataTable
            title="Vendor Addresses"
            subtitle={`Manage addresses for vendor ID: ${vendorId}`}
